...
<!--            <el-table-column prop="sendId" label="申请ID" sortable ></el-table-column>-->
            ...
            <el-table-column label="PDF文件" width="" align="center" >
                <template slot-scope="scope">
                    <el-popconfirm
                            ...
                            icon-color="red"
                            title="确定下载吗"
                            @confirm="download(scope.row.originFileUrl)"
                    >
                        <el-button type="success" slot="reference" >下载</el-button>
                    </el-popconfirm>
                </template>
            </el-table-column>
            <el-table-column prop="urlPath" label="用章图片" width="">
                <template slot-scope="scope">
            ...
            <el-table-column prop="state" label="状态" sortable>
                <template slot-scope="scope">
                    ...
                    <el-tag type="success" v-if="scope.row.state === '2'">加印完成</el-tag>
                    <el-tag type="error" v-if="scope.row.state === '3'">作废</el-tag>
                </template>
            </el-table-column>

            <el-table-column prop="createTime" label="创建时间" ></el-table-column>
            <el-table-column prop="approverName" label="审批人" ></el-table-column>
            <el-table-column prop="apprTime" label="审批时间" ></el-table-column>
            <el-table-column prop="overName" label="用印人" ></el-table-column>
            ...
            <el-table-column label="操作" width="300px" align="center" fixed="right">
                <template slot-scope="scope">
                    ...
                    >
                        <el-button type="danger" slot="reference" v-if="scope.row.state === '0' && user.roleId !== 3 && user.roleId !== 4">作废 <i
                    ...
                    <el-popconfirm
                            ...
                            icon-color="red"
                            title="确定审批吗"
                            @confirm="approve(scope.row.sendId)"
                    >
                        <el-button type="warning" slot="reference" v-if="user.roleId === 1 || user.roleId === 3">审批 <i
                                class="el-icon-remove-outline"></i></el-button>
                    </el-popconfirm>
                    ...
                    <el-popconfirm
                            ...
                            icon-color="red"
                            title="确定用印吗"
                            @confirm="usingSeal(scope.row.sendId)"
                    >
                        <el-button type="success" slot="reference" v-if="user.roleId === 1 || user.roleId === 4">用印 <i
                                class="el-icon-remove-outline"></i></el-button>
                    </el-popconfirm>
                    <el-popconfirm
                            class="ml-5"
                            ...
                            icon-color="red"
                            title="确定下载用印后文件吗"
                            @confirm="downloadSeal(scope.row.newFileUrl)"
                    >
                        <el-button type="success" slot="reference" v-if="scope.row.state === '2'">下载用印后文件 <i
                                class="el-icon-remove-outline"></i></el-button>
                    </el-popconfirm>
                </template>
            </el-table-column>
        ...
        <el-dialog title="申请签章信息" :visible.sync="dialogFormVisible" width="40%">
            <el-form label-width="100px" size="small" :rules="rules" :model="form" ref="from">
                <el-form-item label="申请标题" prop="sendTitle">
                    <el-input placeholder="申请标题唯一" v-model="form.sendTitle" autocomplete="off" style="width: 300px"></el-input>
                </el-form-item>
        ...
        data() {
            return {
                ...
                rules: {
                    ...
                    sealId: [
                        {required: true, message: '请选择用章名称', trigger: 'blur'},
                    ],
                    originFileUrl: [
                        {required: true, message: '请上传用章PDF文件', trigger: 'blur'},
                    ],
                    remark: [
        ...
        methods: {
            ...
            handleAvatarSuccess(res) {
                if (res.codeMsg === 200) {
                    ...
                    this.form.originFileSha = res.fileSha
                    this.$forceUpdate()
            ...
            approve(assId) {
                this.request.delete("/sendSeal/approve/" + assId).then(res => {
                    if (res.code === '200') {
            ...
            usingSeal(assId) {
                this.request.delete("/sendSeal/usingSeal/" + assId).then(res => {
                    if (null != res.code && res.code === '200') {
                        this.$message.success("用印成功")
                        this.load()
                    } else {
                        this.$message.error(res.msg)
                    }
